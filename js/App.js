...
let BAG_DATA_URL = 'http://devsteam.mobi/pcapp/pcbags/mobileapi/?username=pcadmin&authkey=pcadmin123';
...
class App extends Component {
  ...
  async getData() {
    ...
    .then((responseJson) => {
      AsyncStorage.setItem('app', JSON.stringify(responseJson.data));
    })
    .catch((error) => {
      console.error(error);
    });    
  ...
  async getBagData() {
    return await fetch(BAG_DATA_URL)
    .then((response) => response.json())
    .then((responseJson) => {
      AsyncStorage.setItem('bag', JSON.stringify(responseJson.data));
    })
    .catch((error) => {
      console.error(error);
    });    
  ...
  setUser() {
     AsyncStorage.getItem('user', (err, result) => {
      ...
      if(data && data.user) {
        window.userInfo = data.user.customer_info
      }
    });
  }
  componentDidMount() {
