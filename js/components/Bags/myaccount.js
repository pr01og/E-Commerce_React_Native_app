...
class Header3 extends Component {  // eslint-disable-line
  ...
  getCustomerDetails() {
     let url = `http://www.devsteam.mobi/pcapp/index.php/mobileapi/customer/myaccount/?username=pcadmin&authkey=pcadmin123&userid=25`
      return fetch(url)
  ...
  saveAccount(){
    let url = `http://www.devsteam.mobi/pcapp/index.php/mobileapi/customer/updatecustomer/?username=pcadmin&authkey=pcadmin123&userid=24&firstname=${this.state.fname}&&lastname=${this.state.lname}&&email=${this.state.email}`;
    return fetch(url)
    .then((response) => response.json())
    .then((response) => {
      if(response.code === 200){
  ...
  saveAddress(){
    let url = `http://www.devsteam.mobi/pcapp/index.php/mobileapi/checkout/updateaddres/?device_id=358958061090038&customer_id=${this.state.user_id}&address_id=${this.state.address_id}&firstname=${this.state.fname}&lastname=${this.state.fname}&street=${this.state.street}&city=${this.state.city}&country_id=${this.state.country}&region=${this.state.region}&postcode=${this.state.postcode}&telephone=${this.state.phone}&is_default_billing=1&is_default_shipping=1&username=pcadmin&authkey=pcadmin123`;
    return fetch(url)
    .then((response) => response.json())
    .then((response) => {
  ...
  render() {
    ...
    return (
      <Container>
        <Header  style={{ backgroundColor: "#63254d"}}  androidStatusBarColor="#5bc0de" iosBarStyle="light-content">
          <Left>
          ...
          <Body>
            <Title style={{color: "#fff", fontSize:12, fontWeight: '800'}}>MY ACCOUNT</Title>
          </Body>
          <Right />
        </Header>
        { !this.state.logged && 
