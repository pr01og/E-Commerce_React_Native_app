...
import { Container, Header, Title, Content, View, Button, Icon, Left, Right, Body, Text } from 'native-base';
...
class Header5 extends Component {  // eslint-disable-line
  ...
  getWishlist(){
    let url = `http://www.devsteam.mobi/pcapp/index.php/mobileapi/wishlist/wishlist/?device_id=358958061090038&username=pcadmin&authkey=pcadmin123&customer_id=${this.state.user_id}`
     return  fetch(url)
       ...
       items.push(<View key={i}>
          <View style={{ flexDirection: 'row',backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "lightgray"}}>
            <View style={{ alignItems: 'center', width: width/2.4, marginTop: 10, marginBottom: 10}}>
              <Thumbnail square style={{width: width/4, height: height/5}} source={{uri: item.product.mainimage}} />
            </View>
    ...
    return (
      <Container>
        <Header style={{ backgroundColor: "#63254d"}} androidStatusBarColor="#dc2015" iosBarStyle="light-content">
          ...
          <Body>
            <Title style={{color: "#fff", fontSize: 12}}>WISHLISTS</Title>
          </Body>
          <Right />
        </Header>
        <Content padder>
