...
class Brand extends Component {
    ...
    for (let i = 0; i < arry.length; i++) {
      ...
      col.push(
        <Col style={{ flexDirection: 'column', alignItems: "center",   width:  width/3.13, height: responsiveHeight(20), margin: 2, borderWidth: 0.5, borderColor: "#C0C0C0" }} key={i + 100}
          ...
          <Image style={{ flex: 1, height: height/5, width: width/3.5, resizeMode: 'contain' }} source={{ uri: c.image }} />
          <Text style={{margin: 10, fontSize: 10}}>{c.title}</Text>
        </Col>
      );
    }
    return col;
  ...
  render() {
    let heightCard = this.props.brands.length > 6 ?  300 : 300/2
    return (
      <Swiper paginationStyle={{bottom: -20}}  height={heightCard} horizontal={true}>
        { this.renderGridView()}
      </Swiper>
    );
  }
